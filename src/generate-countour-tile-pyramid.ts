...
program
  ...
  .description('Generates a pyramid of contour tiles from a source DEM using the mlcontour library.')
  ...
  .requiredOption('--demUrl <string>', "The URL of the DEM source (can be a PMTiles URL: 'pmtiles://...') or a regular tile URL pattern.")
  .option(
    '--sEncoding <string>',
    "The encoding of the source DEM tiles (e.g., 'terrarium', 'mapbox').",
    (value) => {
      if (value !== 'mapbox' && value !== 'terrarium') {
        throw new Error("Invalid value for --sEncoding, must be 'mapbox' or 'terrarium'")
      }
      return value
    },
  ...
  .requiredOption('--oDir <string>', 'The output directory where tiles will be stored.')
  .parse(process.argv)

const options = program.opts()
const { x, y, z, demUrl, sEncoding, sMaxZoom, increment, oMaxZoom, oDir } = options
const numX = Number(x)
const numY = Number(y)
const numZ = Number(z)
...
async function processTile(v: Tile): Promise<void> {
  ...
  return manager.fetchContourTile(z, x, y, tileOptions, new AbortController()).then((tile) => {
...
async function processQueue(queue: Tile[], batchSize: number = 25): Promise<void> {
  for (let i = 0; i < queue.length; i += batchSize) {
    const batch = queue.slice(i, i + batchSize)
    console.log(`Processing batch ${i / batchSize + 1} of ${Math.ceil(queue.length / batchSize)} of tile ${z}/${x}/${y}`)
    await Promise.all(batch.map(processTile))
    console.log(`Processed batch ${i / batchSize + 1} of ${Math.ceil(queue.length / batchSize)} of tile ${z}/${x}/${y}`)
  }
}
